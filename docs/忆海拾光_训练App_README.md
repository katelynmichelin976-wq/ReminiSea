...
- 首页牌组点击只切换选中态（红色高亮 + 左侧 accent 条），不再跳转到详情页；移除右侧导航箭头
- 修复 v5.11.1 删左滑时连带去掉 click handler 导致选中无响应
- 登录按钮与密码框间距统一
- 牌组管理页本地/云端 Tab 统一为无圆角平铺风格

### v5.11.1 — 2026-06-08

**UI 简化（advanced/standard 模式分流）**

- 首页删除「+ 新建」按钮、牌组卡片左滑操作（移至牌组管理页本地 Tab）
- 我的页删除「统计」入口（Tab Bar「统计」替代）
- 账户页删除「云端牌组」section（牌组管理页「云端 Tab」替代）
- 取消同步开始时「正在同步」语音播报
- Tab Bar 模式分流：standard 模式简化为「首页/练习/我的」3 项；advanced 模式保持「首页/牌组/练习/统计/我的」5 项不变
