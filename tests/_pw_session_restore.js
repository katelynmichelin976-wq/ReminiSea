...
(async () => {
  ...
  try {
    ...
    await ctx1.addInitScript((backup) => {
      localStorage.setItem('yh:v1:session:backup', backup);
    }, FAKE_BACKUP);
    ...
    const noBackup = await page2.evaluate(() => !localStorage.getItem('yh:v1:session:backup'));
    ...
    await ctx4.addInitScript(() => {
      localStorage.setItem('yh:v1:session:backup', 'corrupted{{{json');
    });
    ...
    // 验证辅助信息
    const backupRaw = await page4.evaluate(() => localStorage.getItem('yh:v1:session:backup'));
