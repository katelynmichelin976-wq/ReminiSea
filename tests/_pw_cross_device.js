...
(async () => {
  ...
  try {
    ...
    // ════ PHASE Easy Sync: A→B EasyState 跨设备传播 ════
    section('PHASE Easy Sync: A→B EasyState 跨设备传播');

    // 切换 A 到 easy 模式，session_size = 5
    await run(pageA, () => {
      setSrsMode('easy');
      SRS_CONFIG.easy_session_size = 5;
      SRS_CONFIG.easy_retry_on_wrong = true;
    });

    // 重置 A 每日进度，让 easy 模式能启动
    await run(pageA, () => {
      const today = new Date().toISOString().slice(0, 10);
      localStorage.setItem('yihaiDailyProgress', JSON.stringify({
        date: today, reviewed_today: 0, daily_new_today: 0,
    ...
    pass('Easy Sync: 设备 A 进入练习屏', await run(pageA, () =>
      document.getElementById('screen-quiz')?.classList.contains('active')
    ));
    ...
    for (let i = 0; i < 25; i++) {
      ...
      if (!answered) { await wait(pageA, 300); continue; }
      ...
      await wait(pageA, 600);
      const isRev = await run(pageA, () =>
    ...
    pass('Easy Sync: 设备 A easy session > 0 张', easyAnsweredA > 0);
    await run(pageA, () => goHome());
    await wait(pageA, 500);

    // 验证 A 本地 EasyState 已写入 IDB
    const easyStatesA = await run(pageA, (key) => getAllEasyStates(key), deckKeyA);
    pass('Easy Sync: 设备 A EasyState 写入 IDB', easyStatesA.length > 0);

    // A 同步（trials 上传 → DB trigger 写 easy_card_states）
    await run(pageA, () => runSync({ modal: false, decks: false }));
    await waitSyncDone(pageA, 60000);
    await wait(pageA, 3000);

    // 确认 easy_card_states 云端有数据
    const cloudEasyCount = await run(pageA, async (key) => {
      try {
        const { data } = await _sb.from('easy_card_states')
          .select('card_id').eq('deck_key', key).limit(50);
        return data ? data.length : 0;
      } catch (e) { return 0; }
    }, deckKeyA);
    pass('Easy Sync: 云端 easy_card_states 有记录', cloudEasyCount > 0);

    // 设备 B：清 easy 水位，强制 pull
    await run(pageB, () => {
    ...
    // 验证 B IDB easyCardStates 收到 A 写入的记录
    const easyStatesB = await run(pageB, (key) => getAllEasyStates(key), deckKeyA);
    pass('Easy Sync: 设备 B 收到 A 写入的 EasyState', easyStatesB.length > 0);
    pass('Easy Sync: 设备 B history 含 1（A 答对的）',
      easyStatesB.some(s => (s.history || []).includes(1))
    );
    pass('Easy Sync: 设备 B last_warmup 仅本地 = 0',
      easyStatesB.every(s => (s.last_warmup || 0) === 0)
    );

    // 恢复 A 为 normal 模式
    await run(pageA, () => setSrsMode('normal'));
