...
This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository. Codex should use `AGENTS.md` instead of this file.
...
| `tests/_pw_tts.js` | TTS 核心调用 + 浏览模式 + 语音设置/辅助函数存在性（10 断言，~48 函数覆盖，无需登录） |
...
**当前版本：v5.13.19**（`index.html`，线上版）。**完整变更日志见 `docs/yihai_变更记录_CLAUDE参考.md`——本文件不再记录版本变更**。
